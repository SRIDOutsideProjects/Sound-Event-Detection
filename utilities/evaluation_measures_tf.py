...
def get_predictions(model, dataloader, data_set, decoder, pooling_time_ratio=1, thresholds=[0.5],
    ...
    for i, ((input_data, _), indexes) in enumerate(dataloader):
        ...
        input_data = tf.expand_dims(input_data,axis=3)
        pred_strong, _ = model(input_data)
        pred_strong = pred_strong
        pred_strong = pred_strong.numpy()
        if i == 0:
        ...
        for j, pred_strong_it in enumerate(pred_strong):
            for threshold in thresholds:
                ...
                pred["filename"] = data_set.filename.iloc[indexes[j]]

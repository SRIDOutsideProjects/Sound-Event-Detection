    ...
    def transform_data(self, data):
        """ Apply the transformation on data
            ...
            Returns:
                ...
                The transformed data
        """
        res_data = tf.convert_to_tensor(data)
        if self.unsqueeze_axis is not None:
            res_data = res_data.unsqueeze(self.unsqueeze_axis)
        return res_data

    def transform_label(self, label):
        return tf.convert_to_tensor(label)  # float otherwise error
...
def get_transforms(frames, scaler=None, add_axis=None, add_teacher=False,

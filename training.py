...
from utilities.utils import SaveBest, weights_init, AverageMeterSet, EarlyStopping, \
    get_durations_df
...
for c_epoch in range(epochs):
    ...
    for (batch_whole_1,batch_whole_2) in (zip(dataset1,dataset2)):
        ...
        crnn_ema.model = update_ema_variables(crnn.model,crnn_ema.model,0.999,i)
        ...
        if i%100==0:
            optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=0.005*0.95**(c_epoch+1))
            crnn.model.save('weights/crnn_final_model.h5')
